...
const Submit = () => {
  ...
  const wilayas = ["الجزائر العاصمة", "وهران", "قسنطينة", "عنابة", "البليدة", "باتنة", "سطيف", "سيدي بلعباس", "بسكرة", "تلمسان", "أدرار", "الشلف", "الأغواط", "أم البواقي", "بجاية", "بشار", "البويرة", "تمنراست", "جيجل", "الجلفة", "خنشلة", "العربي بن مهيدي", "ولايات أخرى"];
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        toast({ title: "خطأ في الصورة", description: "الحجم كبير جدًا", variant: "destructive" });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!phoneRegex.test(contactNumber.replace(/\s/g, ""))) {
      toast({ title: "رقم غير صحيح", description: "مثال: +213 555 123 456", variant: "destructive" });
      setIsSubmitting(false);
      return;
    }
    ...
    if (image) {
      ...
      const { error: uploadError } = await supabase.storage.from("ads-images").upload(fileName, image);
      if (uploadError) {
    ...
    setFormData({ type: "lost", itemName: "", description: "", location: "", date: "", contactNumber: "", image: null });
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label>الوصف</Label>
                  <Textarea className="min-h-[100px]" value={formData.description} onChange={(e) => handleInputChange("description", e.target.value)} />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="tel" value={formData.contactNumber} onChange={(e) => handleInputChange("contactNumber", e.target.value)} />
                </div>
